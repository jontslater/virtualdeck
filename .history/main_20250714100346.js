...
function registerHotkeys() {
  ...
  config.buttons.forEach((btn) => {
    if (btn.hotkey) {
      ...
      // Also register numpad version for single number keys
      if (/^\d$/.test(btn.hotkey)) {
        // For single digits, also try registering with numpad prefix
        const numpadHotkey = `NumPad${btn.hotkey}`;
        const numpadSuccess = globalShortcut.register(numpadHotkey, () => {
          win.webContents.send('trigger-media', btn.label);
        });
        
        if (numpadSuccess) {
          console.log(`Registered numpad hotkey: ${numpadHotkey} for ${btn.label}`);
        }
      }
    }
  });
