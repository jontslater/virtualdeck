  ...
  config.buttons.forEach((btn) => {
    if (btn.hotkey) {
      ...
      if (/^F\d{1,2}$/.test(btn.hotkey)) {
        ...
        fKeyFormats.forEach(format => {
          if (!registered) {
            ...
            if (success) {
              console.log(`Registered F-key hotkey: ${format} for ${btn.label}`);
              registered = true;
            }
          }
        });
        ...
        if (!registered) {
          console.warn(`Failed to register F-key hotkey: ${btn.hotkey} for ${btn.label}`);
        }
      }
      // Handle single number keys (1-0)
        ...
        numberFormats.forEach(format => {
          if (!registered) {
            try {
              ...
              if (success) {
                console.log(`Registered number hotkey: ${format} for ${btn.label}`);
                registered = true;
              }
            } catch (error) {
        ...
        if (!registered) {
          console.warn(`Failed to register number hotkey: ${btn.hotkey} for ${btn.label}`);
        }
      }
