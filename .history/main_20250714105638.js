...
ipcMain.on('disable-hotkeys', () => {
  ...
  console.log('All hotkeys disabled (modal open)');
});
ipcMain.on('enable-hotkeys', () => {
  registerHotkeys();
  console.log('Hotkeys re-enabled (modal closed)');
});

function registerHotkeys() {
  // Unregister all existing hotkeys first to avoid conflicts
  globalShortcut.unregisterAll();
  console.log('🔄 Unregistered all existing hotkeys');
    ...
    if (btn.hotkey) {
      ...
      if (/^F\d{1,2}$/.test(btn.hotkey)) {
        ...
        if (!registered) {
          ...
          // Special warning for F1 which is often reserved
          if (btn.hotkey === 'F1') {
            console.warn(`⚠️  F1 is often reserved by the OS or other applications. Try using F2-F12 instead.`);
          }
        }
      }
